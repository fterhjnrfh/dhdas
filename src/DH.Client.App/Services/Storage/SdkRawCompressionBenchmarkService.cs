...
internal static class SdkRawCompressionBenchmarkService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return snapshot.PayloadKind == CompressionPayloadKind.RawCaptureBlockPayload
            ? BuildRawCapturePayloadRows(
                snapshot,
                capturePath,
                replayMode,
    ...
    private static IReadOnlyList<CompressionBenchmarkRow> BuildTdmsPayloadRows(
        CompressionSessionSnapshot snapshot,
        string capturePath,
        ...
        long totalBlocks,
        Action<CompressionBenchmarkProgress>? progressCallback,
        CancellationToken cancellationToken)
    {
        ...
        var progressReporter = new ProgressReporter(
            ...
            totalCandidateChannelCount,
            totalBlocks,
            totalTargetSamples,
            targetsFullyKnown,
            () => accumulators.Length == 0 ? 0L : accumulators[0].BatchCount);
    ...
    private static IReadOnlyList<CompressionBenchmarkRow> BuildRawCapturePayloadRows(
        CompressionSessionSnapshot snapshot,
        string capturePath,
        CompressionBenchmarkReplayMode replayMode,
        int totalCandidateChannelCount,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var replayedSamplesPerChannel = selectedChannelIds.ToDictionary(
            ...
            _ => 0L);
        var accumulators = CompressionBenchmarkDefaults.Algorithms
            .Select(static algorithm => new RawCaptureAlgorithmAccumulator(algorithm))
            .ToArray();
        var progressReporter = new ProgressReporter(
            progressCallback,
            snapshot.PayloadKind,
            ...
            totalTargetSamples,
            targetsFullyKnown,
            () => accumulators.Length == 0 ? 0L : accumulators[0].EncodedBlockCount);
        ...
        while (SdkRawCaptureConverter.TryReadRawBlock(reader, fileHeader, out var rawBlock))
        {
            try
            {
                ...
                blocksProcessed++;
                samplesProcessed += ReplayRawBlockAsCapturePayload(
                    rawBlock,
                    selectedChannelSet,
                    targetSamplesPerChannel,
                    replayedSamplesPerChannel,
        ...
        if (accumulators.All(static accumulator => accumulator.EncodedBlockCount == 0))
        {
            return Array.Empty<CompressionBenchmarkRow>();
        }
    ...
    private static long ReplayRawBlockAsCapturePayload(
        SdkRawBlock rawBlock,
        IReadOnlySet<int> selectedChannelSet,
        IReadOnlyDictionary<int, long> targetSamplesPerChannel,
        IDictionary<int, long> replayedSamplesPerChannel,
        IReadOnlyList<RawCaptureAlgorithmAccumulator> accumulators,
        CompressionSessionSnapshot snapshot,
        ref int completedChannelCount,
        CancellationToken cancellationToken)
    {
        int deviceId = SdkRawCaptureWriter.ResolveChannelDeviceId(rawBlock.GroupId, rawBlock.MachineId);
        ReadOnlySpan<float> payload = rawBlock.PayloadSpan;
        if (payload.IsEmpty || rawBlock.ChannelCount <= 0 || rawBlock.DataCountPerChannel <= 0)
        {
            return 0L;
        }

        var selectedChannels = new List<SelectedChannelSegment>(rawBlock.ChannelCount);
        for (int channelIndex = 0; channelIndex < rawBlock.ChannelCount; channelIndex++)
        {
            int channelId = ChannelNaming.MakeChannelId(deviceId, channelIndex + 1);
            if (!selectedChannelSet.Contains(channelId)
                || !replayedSamplesPerChannel.TryGetValue(channelId, out long replayedSamples))
            {
                continue;
            }
            ...
            selectedChannels.Add(new SelectedChannelSegment(channelIndex, channelId));
        }
        ...
        long replayedSamplesThisBlock = 0L;
        int sampleOffset = 0;
        while (sampleOffset < rawBlock.DataCountPerChannel)
        {
            cancellationToken.ThrowIfCancellationRequested();

            var activeChannels = new List<SelectedChannelSegment>(selectedChannels.Count);
            int sampleCount = rawBlock.DataCountPerChannel - sampleOffset;
            foreach (var channel in selectedChannels)
            {
                long replayedSamples = replayedSamplesPerChannel[channel.ChannelId];
                long targetSamples = targetSamplesPerChannel.TryGetValue(channel.ChannelId, out long target)
                    ? target
                    : long.MaxValue;
                long remainingSamples = targetSamples - replayedSamples;
                ...
                sampleCount = Math.Min(sampleCount, (int)Math.Min(remainingSamples, int.MaxValue));
            }

            if (activeChannels.Count == 0 || sampleCount <= 0)
            {
                break;
            }

            float[] blockPayload = ExtractSelectedPayload(
                payload,
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                accumulator.RecordBlock(
                    blockPayload,
                    activeChannels.Count,
                    sampleCount,
                    snapshot.PreprocessType,
                    snapshot.CompressionOptions);
            }

            foreach (var channel in activeChannels)
            {
                long previousSamples = replayedSamplesPerChannel[channel.ChannelId];
                long updatedSamples = previousSamples + sampleCount;
                long targetSamples = targetSamplesPerChannel.TryGetValue(channel.ChannelId, out long target)
                    ? target
                    : long.MaxValue;
                replayedSamplesPerChannel[channel.ChannelId] = updatedSamples;
                if (previousSamples < targetSamples && updatedSamples >= targetSamples)
                {
                    completedChannelCount++;
                }
            }
            ...
            replayedSamplesThisBlock += (long)activeChannels.Count * sampleCount;
        }

        return replayedSamplesThisBlock;
    }

    private static float[] ExtractSelectedPayload(
        ReadOnlySpan<float> payload,
        int sourceChannelCount,
        int sampleOffset,
        int sampleCount,
        IReadOnlyList<SelectedChannelSegment> channels)
    {
        if (sampleOffset == 0 && sampleCount * sourceChannelCount == payload.Length && channels.Count == sourceChannelCount)
        {
            return payload.ToArray();
        }

        int targetChannelCount = channels.Count;
        var extracted = new float[targetChannelCount * sampleCount];
        int writeIndex = 0;
        for (int sampleIndex = 0; sampleIndex < sampleCount; sampleIndex++)
        {
            int sourceBaseIndex = (sampleOffset + sampleIndex) * sourceChannelCount;
            for (int channelIndex = 0; channelIndex < targetChannelCount; channelIndex++)
            {
                extracted[writeIndex++] = payload[sourceBaseIndex + channels[channelIndex].ChannelIndex];
            }
        }
    ...
    private readonly record struct SelectedChannelSegment(int ChannelIndex, int ChannelId);

    private sealed class ProgressReporter
    {
        private readonly Action<CompressionBenchmarkProgress>? _callback;
        private readonly CompressionPayloadKind _payloadKind;
        ...
        public ProgressReporter(
            ...
            long totalTargetSamples,
            bool targetsFullyKnown,
            Func<long> encodedUnitCountProvider)
        {
            _callback = callback;
            ...
            _encodedUnitCountProvider = encodedUnitCountProvider;
        }
        ...
        public void Report(long blocksProcessed, long samplesProcessed, bool force = false, bool completed = false)
        {
            ...
            long encodedUnitCount = _encodedUnitCountProvider();
            string encodedUnitLabel = _payloadKind == CompressionPayloadKind.RawCaptureBlockPayload
                ? "编码块"
                : "编码批次";
            ...
            string statusText = completed
                ? $"已完成基于已保存原始数据的压缩算法评估（{modeText}，通道 {_selectedChannelCount:N0}/{_totalCandidateChannelCount:N0}，{blockText}，{sampleText}，{encodedUnitLabel} {encodedUnitCount:N0}）。"
                : $"正在基于已保存原始数据评估各压缩算法性能（{modeText}，通道 {_selectedChannelCount:N0}/{_totalCandidateChannelCount:N0}，{blockText}，{sampleText}，{encodedUnitLabel} {encodedUnitCount:N0}，{progressText}）...";
            ...
            _callback(new CompressionBenchmarkProgress
            {
                ...
                EncodedBatchCount = encodedUnitCount,
    ...
    private sealed class AlgorithmAccumulator
    {
        ...
        public CompressionBenchmarkRow ToRow(CompressionSessionSnapshot snapshot)
        {
            return new CompressionBenchmarkRow
            {
                ...
                EstimatedStoredBytes = CompressionBenchmarkService.EstimateStoredBytesForFullDataset(
                    snapshot.RawBytes,
                    _rawBytes,
    ...
    private sealed class RawCaptureAlgorithmAccumulator
    {
        private const int MaxLatencySampleCount = 4096;
        private readonly List<double> _encodeLatencyMsSamples = new();
        private long _encodedBlockCount;
        private long _sampleCount;
        private long _rawBytes;
        private long _payloadBytes;
        private double _encodeSeconds;

        public RawCaptureAlgorithmAccumulator(CompressionType algorithm)
        {
            Algorithm = algorithm;
        }
        ...
        public long EncodedBlockCount => _encodedBlockCount;

        public void RecordBlock(
            ReadOnlySpan<float> blockPayload,
            int channelCount,
            ...
            CompressionOptions compressionOptions)
        {
            ...
            _encodedBlockCount++;
            _sampleCount += (long)channelCount * samplesPerChannel;
            _rawBytes += encodeResult.RawBytes;
            _payloadBytes += encodeResult.PayloadBytes.Length;
            _encodeSeconds += sw.Elapsed.TotalSeconds;
            AppendLatency(_encodeLatencyMsSamples, sw.Elapsed.TotalMilliseconds, _encodedBlockCount);
        }
        ...
        public CompressionBenchmarkRow ToRow(CompressionSessionSnapshot snapshot)
        {
            return new CompressionBenchmarkRow
            {
                ...
                PreprocessType = snapshot.PreprocessType,
                CompressionOptions = snapshot.CompressionOptions.Clone(),
                BatchCount = _encodedBlockCount,
                SampleCount = _sampleCount,
                RawBytes = _rawBytes,
                CodecBytes = _payloadBytes,
                TdmsPayloadBytes = _payloadBytes,
                EstimatedStoredBytes = CompressionBenchmarkService.EstimateStoredBytesForFullDataset(
                    snapshot.RawBytes,
                    _rawBytes,
