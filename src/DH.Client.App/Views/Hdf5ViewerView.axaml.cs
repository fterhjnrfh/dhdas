...
public partial class Hdf5ViewerView : UserControl
{
    public static readonly StyledProperty<string?> SelectedFileProperty =
        AvaloniaProperty.Register<Hdf5ViewerView, string?>(nameof(SelectedFile));

    public static readonly StyledProperty<int> SelectedDeviceIdProperty =
        AvaloniaProperty.Register<Hdf5ViewerView, int>(nameof(SelectedDeviceId), -1);
    ...
    public Hdf5ViewerView()
    {
        InitializeComponent();
        var vm = new ViewModels.Hdf5ViewerViewModel();
        DataContext = vm;

        var skView = this.FindControl<SkiaMultiChannelView>("Hdf5CurveView");
        if (skView is not null)
        {
            ...
            {
                vm.LastView = new ViewModels.Hdf5ViewerViewModel.ViewState
                {
                    ZoomX = state.ZoomX,
    ...
    {
        var skView = this.FindControl<SkiaMultiChannelView>("Hdf5CurveView");
        if (skView is null || DataContext is not ViewModels.Hdf5ViewerViewModel vm)
        {
            return;
        }
    ...
    private async void OnPickFileClicked(object? sender, RoutedEventArgs e)
    {
        var dialog = new OpenFileDialog
        {
            ...
            {
                new() { Name = "HDF5 Files", Extensions = new List<string> { "h5", "hdf5" } },
