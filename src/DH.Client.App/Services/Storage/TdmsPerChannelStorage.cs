...
namespace DH.Client.App.Services.Storage
{
    public class TdmsPerChannelStorage : ITdmsStorage
    {
        ...
        public void Write(int channelId, ReadOnlySpan<double> samples)
        {
            ...
            object channelLock = _channelLocks.GetOrAdd(channelId, static _ => new object());
            lock (channelLock)
            {
                if (!_started)
                {
                ...
                {
                    string safeName = SanitizeName(_sessionName ?? "session");
                    ...
                    if (File.Exists(path))
                    {
                        ...
                        {
                            path = Path.Combine(_basePath ?? ".", $"{baseFile}_{DateTime.Now:yyyyMMdd_HHmmss}.tdms");
                        }
                    }
                    ...
                    if (err != 0)
                    {
                        throw new IOException($"DDC_CreateFile failed, channel={channelId}: {err} {TdmsNative.DescribeError(err)}");
                    }
                    ...
                    if (err != 0)
                    {
                        throw new IOException($"DDC_AddChannelGroup failed, channel={channelId}: {err} {TdmsNative.DescribeError(err)}");
                    }
        ...
        public void Flush()
        {
            lock (_globalLock)
            {
                foreach (var kvp in _channels)
                {
                    int id = kvp.Key;
                    IntPtr ch = kvp.Value;
                    object channelLock = _channelLocks.GetOrAdd(id, static _ => new object());
                    lock (channelLock)
                    {
